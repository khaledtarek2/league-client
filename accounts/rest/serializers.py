...
from champions.rest.serializers import ChampionSerializer, ChampionSkinSerializer, EnumSerializer
from ..models import Rank, Profile, PlayerSeasonGrade, Player, Message
from rest_framework.exceptions import  ValidationError

class RankSerializer(serializers.ModelSerializer):
    type = EnumSerializer(enum=Rank.Types)
...
class ChampionOwnerShipSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        if ChampionOwnerShip.objects.filter(player_id=player_id, champion_id=champion_id).exists():
            raise ValidationError({'message': 'the champion aleady exists in your account'})
        return validated_data
...
class MessageSerializer(serializers.ModelSerializer):
    ...
    sender = serializers.SlugRelatedField(many=False, slug_field='username', queryset=Player.objects.all())
    receiver = serializers.SlugRelatedField(many=False, slug_field='username', queryset=Player.objects.all())
    class Meta:
        model = Message
        fields = ['sender', 'receiver', 'message', 'timestamp']ّّّ